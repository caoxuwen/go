...
import (
	...
	"github.com/caoxuwen/go-xdr/xdr3"
)
...
const (
	...
	OfferEntryFlagsMarginFlag  OfferEntryFlags = 2
)

var offerEntryFlagsMap = map[int32]string{
	1: "OfferEntryFlagsPassiveFlag",
	2: "OfferEntryFlagsMarginFlag",
}
...
// CreateMarginOfferOp is an XDR Struct defines as:
//
...
// MarshalBinary implements encoding.BinaryMarshaler.
func (s CreateMarginOfferOp) MarshalBinary() ([]byte, error) {
	b := new(bytes.Buffer)
	_, err := Marshal(b, s)
	return b.Bytes(), err
}
...
// UnmarshalBinary implements encoding.BinaryUnmarshaler.
func (s *CreateMarginOfferOp) UnmarshalBinary(inp []byte) error {
	_, err := Unmarshal(bytes.NewReader(inp), s)
	return err
}
...
func NewOperationBody(aType OperationType, value interface{}) (result OperationBody, err error) {
	...
	case OperationTypeCreateMarginOffer:
		tv, ok := value.(CreateMarginOfferOp)
		if !ok {
			err = fmt.Errorf("invalid value, must be CreateMarginOfferOp")
			return
		}
...
// MustCreateMarginOfferOp retrieves the CreateMarginOfferOp value from the union,
// panicing if the value is not set.
func (u OperationBody) MustCreateMarginOfferOp() CreateMarginOfferOp {
	val, ok := u.GetCreateMarginOfferOp()

	if !ok {
		panic("arm CreateMarginOfferOp is not set")
	}

	return val
}

// GetCreateMarginOfferOp retrieves the CreateMarginOfferOp value from the union,
// returning ok if the union's switch indicated the value is valid.
func (u OperationBody) GetCreateMarginOfferOp() (result CreateMarginOfferOp, ok bool) {
	armName, _ := u.ArmForSwitch(int32(u.Type))

	if armName == "CreateMarginOfferOp" {
		result = *u.CreateMarginOfferOp
		ok = true
	}
...
// MustCreateMarginOfferResult retrieves the CreateMarginOfferResult value from the union,
// panicing if the value is not set.
func (u OperationResultTr) MustCreateMarginOfferResult() ManageOfferResult {
	val, ok := u.GetCreateMarginOfferResult()

	if !ok {
		panic("arm CreateMarginOfferResult is not set")
	}

	return val
}

// GetCreateMarginOfferResult retrieves the CreateMarginOfferResult value from the union,
// returning ok if the union's switch indicated the value is valid.
func (u OperationResultTr) GetCreateMarginOfferResult() (result ManageOfferResult, ok bool) {
	armName, _ := u.ArmForSwitch(int32(u.Type))
