...
The `compliance.cfg` file must be present in a working directory (you can load another file by using `-c` parameter). Here is an [example configuration file](https://github.com/caoxuwen/bridge-server/blob/master/compliance_example.cfg). Config file should contain following values:
...
```
### `callbacks.tx_status`
This callback should return the status of a transaction as explained in [`SEP-0004`](https://github.com/caoxuwen/stellar-protocol/blob/master/ecosystem/sep-0004.md).
